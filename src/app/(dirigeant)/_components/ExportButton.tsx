...
/**
 * Bouton d'export CSV réel pour l'espace dirigeant.
...
export function ExportButton({
  ...
  headers: string[];
  rows: Cell[][];
  /** « ghost » conserve le rendu GhostButton, « gold » celui du GoldButton. */
  variant?: "ghost" | "gold";
}) {
  ...
  const className =
    variant === "gold"
      ? "rounded-md bg-[var(--gold)] px-3 py-2 text-[11.5px] font-bold text-white hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-50"
      : "rounded-md border border-[var(--navy-100)] bg-white px-3 py-2 text-[11.5px] font-semibold text-[var(--navy)] hover:border-[var(--gold)] disabled:cursor-not-allowed disabled:opacity-50";
