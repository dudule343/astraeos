...
export default function SimulateursPage() {
  return (
    <PageScaffold
      eyebrow="Outils · simulateurs & calculateurs"
      title="Simulateurs & calculateurs"
      description="Outils mis à disposition des cabinets licenciés et des clients : simulations fiscales, financières et immobilières."
    >
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
        {SIMULATEURS.map((sim) => (
          <div
            key={sim.title}
            className="flex flex-col rounded-lg border border-[var(--navy-100)] border-l-4 border-l-[var(--gold)] bg-white"
          >
            <div className="flex min-h-[260px] flex-1 flex-col p-5">
              <div className="mb-3.5 flex items-center gap-3.5">
                <div className="grid h-12 w-12 flex-shrink-0 place-items-center rounded-[10px] border-[1.5px] border-[var(--gold)] bg-[var(--ivory)] text-[var(--gold-deep)]">
                  {sim.icon}
                </div>
                <strong className="text-[15px] text-[var(--navy)]">{sim.title}</strong>
              </div>
              <p className="flex-1 text-[12px] leading-relaxed text-[var(--navy-300)]">
                {sim.description}
              </p>
              <button
                type="button"
                ...
                className="mt-[18px] w-full cursor-not-allowed rounded-md bg-[var(--gold)] px-4 py-3 text-[13px] font-bold text-white opacity-50"
              >
